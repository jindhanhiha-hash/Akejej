...
def get_random_proxy() -> Optional[str]:
    ...
    if not available:
        print("⚠️ No working proxies left – falling back to direct (no proxy).")
        return None
    return random.choice(available)

# ---------- Request helpers with proxy retry ----------
async def request_with_retry(method, url, retries=5, **kwargs):
    """Make an aiohttp request with proxy rotation and SSL verification disabled."""
    ...
    for attempt in range(retries):
        ...
        if proxy:
            ...
            kwargs.pop('proxy', None)  # direct if no proxy
...
async def get_bind_info(token: str) -> Dict[str, Any]:
    ...
    # First attempt: direct (no proxy) with longer timeout
    timeout = aiohttp.ClientTimeout(total=30, connect=10, sock_read=20)
    try:
        async with aiohttp.ClientSession() as session:
